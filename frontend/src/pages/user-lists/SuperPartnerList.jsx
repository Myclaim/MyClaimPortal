...
import { ShieldCheck, Search, Download, Eye, Edit2, Trash2, Zap, Phone, Mail, Plus, X } from 'lucide-react';
...
const SuperPartnerList = () => {
  ...
  const handleSavePartner = async () => {
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.message || 'Failed to update super partner');
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setPartners(data.filter((u) => u.role === 'super_partner'));
      } catch (err) {
        console.error('Error fetching super partners:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeletePartner = async (user) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Failed to delete super partner');
    }
  };

  const handleExport = () => {
    if (filtered.length === 0) return alert('No super partners to export');
    const headers = ['Ref ID', 'Master Partner', 'Organization', 'Email', 'Phone', 'Status', 'Clients', 'Joined'];
    const rows = filtered.map(u => [
      u.client_id_ref || `SPR-${parseInt(String(u._id).substring(0,8), 16)}`,
      u.name,
      u.companyName || u.entity || '—',
      u.email,
      u.phone || '',
    ...
    a.href = url;
    a.download = `super_partners_export_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  const PartnerRow = ({ user }) => (
    <tr className="animate-slide-up">
      <td>
        <b style={{ color: 'var(--blue)', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px' }}>{user.client_id_ref || `SPR-${parseInt(String(user._id).substring(0,8), 16)}`}</b>
      </td>
      <td>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-light)', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 500 }}>
                <Zap size={10} className="text-brand-500" fill="currentColor" /> Master Franchise
            </div>
          </div>
        </div>
      </td>
      <td>
        <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>{user.companyName || '—'}</div>
        <div style={{ fontSize: '11px', color: 'var(--text-light)' }}>Network Tier 1</div>
      </td>
      <td>
      ...
      <td>
        <span className="custom-badge badge-purple" style={{ fontWeight: 800 }}>
          {user.clientCount || 0} Clients Managed
        </span>
      </td>
      <td>
      ...
      <td>
        <div className="action-icons">
          <button type="button" className="action-icon" title="View super partner" onClick={() => openViewPartner(user)}><Eye size={16} /></button>
          <button type="button" className="action-icon" title="Edit super partner" onClick={() => openEditPartner(user)}><Edit2 size={16} /></button>
          <button type="button" className="action-icon" title="Delete super partner" style={{ color: '#ef4444' }} onClick={() => handleDeletePartner(user)}><Trash2 size={16} /></button>
        </div>
      </td>
    </tr>
  );
    ...
    <div className="page active" style={{ display: 'block' }}>
      ...
      <div className="topbar">
        <div className="animate-slide-up">
          <div className="topbar-title">Super Partner Directory</div>
          <div className="topbar-subtitle">Elite level partners managing Master Franchises</div>
        </div>
        <div className="topbar-spacer"></div>
        <button className="topbar-btn" onClick={() => navigate('/users/add?role=super_partner')} style={{ background: 'linear-gradient(135deg, #059669 0%, #10b981 100%)', color: '#fff', border: 'none' }}>
          <Plus size={16} /> <span>Add Super Partner</span>
        </button>
      </div>
      ...
      <div style={{ padding: '32px' }}>
        <div className="stats-row cols-3 animate-slide-up" style={{ marginBottom: '24px' }}>
          <div className="stat-card">
            <div className="stat-label">Total Super Partners</div>
            <div className="stat-value">{loading ? '...' : partners.length}</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 700 }}>{partners.filter(p => p.is_active !== false).length} Active</div>
          </div>
          <div className="stat-card">
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 700 }}>Across all partners</div>
          </div>
        </div>
        ...
        <div className="card animate-slide-up" style={{ padding: 0, overflow: 'hidden', animationDelay: '0.2s' }}>
          <div className="search-bar" style={{ padding: '16px', margin: 0, borderBottom: '1px solid var(--border)' }}>
            <div className="search-input" style={{ flex: 1 }}>
              ...
              <input
                type="text"
                placeholder="Search master partners..."
                value={query}
          ...
          <div className="table-wrap">
            <table style={{ width: '100%' }}>
              <thead>
                <tr>
                  <th>Ref ID</th><th>Master Partner</th><th>Organization</th><th>Contact</th><th>Portfolio</th><th>Status</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '64px', color: '#94a3b8' }}>
                      {loading ? 'Consulting Master Node...' : 'No super partners found in registry'}
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                <div>
                  ...
                  <div style={{ marginBottom: 32 }}>
                    <div className="va-row">
                      ...
                      <span className="va-value">{activePartner.client_id_ref || `SPR-${parseInt(String(activePartner._id).substring(0,8), 16)}`}</span>
                    </div>
                    <div className="va-row">
          ...
          <div className="modal" style={{ animation: 'fadeInScale 0.3s forwards', maxWidth: '760px' }} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div className="modal-title" style={{ fontSize: '20px' }}>
                Edit Super Partner — {(editForm.client_id_ref || `SPR-${parseInt(String(editForm._id).substring(0,8), 16)}`)}
              </div>
            ...
            <div className="modal-body" style={{ maxHeight: '65vh', overflowY: 'auto', padding: '32px' }}>
              <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--text-muted)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: '16px' }}>MASTER PARTNER DETAILS</div>
              <div className="form-row cols-2">
                ...
                <div className="form-group">
                  <span className="form-label text-gray">ORGANIZATION</span>
              ...
              <div className="form-row cols-2" style={{ marginTop: 16 }}>
                ...
                <div className="form-group">
                  <span className="form-label text-gray">ENTITY</span>
                  <input className="form-input" value={editForm.entity} onChange={(e) => setEditForm((p) => ({ ...p, entity: e.target.value }))} style={{background:'var(--card)', borderColor:'rgba(255,255,255,0.1)', color:'var(--text)'}} />
                </div>
              </div>
              <div className="form-row cols-2" style={{ marginTop: 16 }}>
                <div className="form-group">
