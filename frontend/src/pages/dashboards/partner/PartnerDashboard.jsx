                  ...
                  <td style={{ padding: '16px 24px' }}>
                    {row.attachmentPath ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <button 
                          onClick={(e) => { e.stopPropagation(); setPreviewDoc({ name: `${row.client} - Proposal Document`, url: row.attachmentPath }); }}
                          style={{
                      ...
                      <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontStyle: 'italic' }}>No file attached</span>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="modal-overlay open" onClick={e => { if (e.target === e.currentTarget) setSelectedLead(null); }}>
          <div className="modal" style={{ animation: 'fadeInScale 0.3s forwards', maxWidth: '980px', width: '95%', background: 'transparent', padding: 0, boxShadow: 'none' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="lead-view-wrap" style={{ maxHeight: '85vh', overflowY: 'auto' }}>
              ...
              <div className="lead-view-section" style={{ marginTop: '20px' }}>
                <div className="lead-view-title">Lead Documents & Attachments</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  {selectedLead.nomineeAadharPath && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.04)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <span style={{ fontSize: '13px', color: '#e2e8f0', fontWeight: 600 }}>Aadhar Card</span>
                      <button 
                        type="button"
                        onClick={() => setPreviewDoc({ name: `${selectedLead.name} - Aadhar`, url: selectedLead.nomineeAadharPath })}
                        style={{ background: 'rgba(59, 130, 246, 0.2)', border: '1px solid #3b82f6', color: '#60a5fa', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                      >
                        <Eye size={14} /> Preview
                      </button>
                    </div>
                  )}
                  {selectedLead.nomineePanPath && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.04)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <span style={{ fontSize: '13px', color: '#e2e8f0', fontWeight: 600 }}>PAN Card</span>
                      <button 
                        type="button"
                        onClick={() => setPreviewDoc({ name: `${selectedLead.name} - PAN`, url: selectedLead.nomineePanPath })}
                        style={{ background: 'rgba(59, 130, 246, 0.2)', border: '1px solid #3b82f6', color: '#60a5fa', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                      >
                        <Eye size={14} /> Preview
                      </button>
                  ...
                  {selectedLead.nomineeNocPath && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.04)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <span style={{ fontSize: '13px', color: '#e2e8f0', fontWeight: 600 }}>NOC Document</span>
                      <button 
                        type="button"
                        onClick={() => setPreviewDoc({ name: `${selectedLead.name} - NOC`, url: selectedLead.nomineeNocPath })}
                        style={{ background: 'rgba(59, 130, 246, 0.2)', border: '1px solid #3b82f6', color: '#60a5fa', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                      >
                        <Eye size={14} /> Preview
                      </button>
                  ...
                  {selectedLead.nomineeOtherDocsPath && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.04)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <span style={{ fontSize: '13px', color: '#e2e8f0', fontWeight: 600 }}>Other Document</span>
                      <button 
                        type="button"
                        onClick={() => setPreviewDoc({ name: `${selectedLead.name} - Attachment`, url: selectedLead.nomineeOtherDocsPath })}
                        style={{ background: 'rgba(59, 130, 246, 0.2)', border: '1px solid #3b82f6', color: '#60a5fa', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                      >
                        <Eye size={14} /> Preview
                      </button>
                  ...
                  {!selectedLead.nomineeAadharPath && !selectedLead.nomineePanPath && !selectedLead.nomineeNocPath && !selectedLead.nomineeOtherDocsPath && (
                    <span style={{ fontSize: '13px', color: '#94a3b8', fontStyle: 'italic', gridColumn: 'span 2' }}>No documents attached to this lead.</span>
                  )}
                </div>
              </div>
    ...
    if (categoryName === 'Pre IPOs') {
      const fetchLivePreIpos = async () => {
        try {
          ...
          console.error('Failed to fetch live Pre-IPOs for partner', err);
        }
      };
      fetchLivePreIpos();
    }
